...
range = "Raw Log!A:C"                                             # Range should contain correct SHEET NAME and COLUMNS
localLogPath = "log.txt"                                          # Path to local file log 
...
def newLogText(logData):
  ...
  textFile.write(logData[0] + "\t" + logData[1] + "\t" + logData[2] + "\n")
...
def main():
  ...
  while choice == "Y":
    ...
    logData.append(log)
    logData.append(datetime.today().strftime("%Y-%m-%d"))
    logData.append(datetime.now().strftime("%H:%M:%S"))
    
    # Print to console for debugging
    print(logData)
    
    # Add log data to local log file, then google sheets
    newLogText(logData)
